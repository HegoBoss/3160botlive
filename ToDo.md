...
- [ ] **Message Handler opzetten**
    ...
    - [ ] Zorgen dat de bot leest wat er geschreven wordt (filteren op berichten die niet van bots zijn).
    - [ ] Basic logica toevoegen: "Als bericht start met `!`, doe iets".
- [ ] **Activatie Command Maken**
	- [ ] wakeup command die mensen move door aantal channels voor ze actief weer te maken 
    - [ ] Een specifiek command maken (bijv. `!starttourney` of `!moveall`).
    - [ ] Zorgen dat alleen admins/mods dit command kunnen gebruiken.
- [ ] **Voice Move Logica**
    - [ ] De code schrijven om een `SocketGuildUser` te verplaatsen naar een ander Voice Channel.
    - [ ] Check inbouwen: Zit de gebruiker wel in een voice channel? (Anders crasht de bot).
...
- [ ] **Discord Permissions Check**
    - [ ] In de Discord Developer Portal: Zorg dat **"Move Members"** aan staat in de bot invite link.
    - [ ] In de Discord Developer Portal: Zet **"Message Content Intent"** AAN (belangrijk om tekst te kunnen lezen!).
